...
def validate_regions(bgen, regions, S):
    ...
    TODO: either fix bgen positions and iterate using iter_variants_in_region or 
    parallelise using multiprocessing or both to speed up.
    Args:
...
def simulate_phenotype(bgen, variants, S):
    ...
    for variant_name in selected:
        ...
        #variants_json = variants_table.to_json(orient='records')
        gt = fix_gt(variant[1])
        y_contrib = eff * gt
...
if __name__ == '__main__':
    ...
    if (len(S.name) <= 0):
        ...
        S.name = "simulation_" + os.path.basename(S.bgen_file_path) + "_" + os.path.basename(S.bed_regions_path) + "_" + now
    ...
    with PyBGEN(S.bgen_file_path) as bgen:
        ...
        exec_time = round(time.time() - start_time, 2)
        print(f" done in {exec_time}s.")
        simulated_phenos = pd.DataFrame({'IID': bgen.samples})
        with open(S.out + "/" + S.name + ".json", "w") as json_file:
            cnt = 1
            for region in sim_regions:
                ...
                json.dump(sim_result['region'], json_file, indent=4)
                json_file.write("\n")
                sim_result['variants_data'].to_csv(json_file, mode='a', header=True, index=False)
                json_file.write("\n")
                #print(sim_result)
                ...
                print(f"\t\t - done in {exec_time}s.")
                cnt += 1
        with open(S.out + "/phenotypes_" + S.name + ".csv", "w") as csv_file:
